		...
		<div class="col-md-12">
			<form class="form-inline" v-on:submit.prevent="getProfitLoss">
				<div class="form-group" style="margin-right: 15px;">
					<select id="searchBy" @change="onChangeSearch" v-model="searchBy" class="form-control" style="padding: 0 4px;width: 125px;height: 28px;">
						<option value="customer">Customer</option>
						<option value="supplier">Supplier</option>
					</select>
				</div>
				<div v-if="searchBy == 'customer'" style="display:none;" :style="{display: searchBy == 'customer' ? '' : 'none'}" class="form-group" style="margin-right: 15px;">
					<label>Customer &nbsp;</label>
					<v-select v-bind:options="customers" v-model="selectedCustomer" label="display_name" placeholder="Select Customer"></v-select>
				</div>
				<div v-if="searchBy == 'supplier'" style="display:none;" :style="{display: searchBy == 'supplier' ? '' : 'none'}" class="form-group" style="margin-right: 15px;">
					<label>Supplier &nbsp;</label>
					<v-select v-bind:options="suppliers" v-model="selectedSupplier" label="display_name" placeholder="Select Supplier"></v-select>
				</div>
	...
	<div class="row" style="display:none;" :style="{display: show_report ? '' : 'none'}">
		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportTable">
				<table id="table1" v-if="searchBy == 'customer'" style="display:none;" :style="{display: searchBy == 'customer' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Product Id</th>
							<th>Product</th>
				...
				<table id="table1" v-if="searchBy == 'supplier'" style="display:none;" :style="{display: searchBy == 'supplier' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Product Id</th>
							<th>Product</th>
					...
					<tbody>
						<tr v-for="product in reportData">
							<td>{{ product.Product_Code }}</td>
							<td>{{ product.Product_Name }}</td>
							<td style="text-align:right;">{{ product.SaleDetails_TotalQuantity }}</td>
							<td style="text-align:right;">{{ product.Purchase_Rate | decimal }}</td>
						...
						<tr style="background-color: #f0f0f0;font-weight: bold;">
							<td colspan="4" style="text-align:right;">Total</td>
							<td style="text-align:right;">{{ reportData.reduce((prev, cur) => { return prev + parseFloat(cur.purchased_amount) }, 0) | decimal }}</td>
							<td style="text-align:right;">{{ reportData.reduce((prev, cur) => { return prev + parseFloat(cur.SaleDetails_TotalAmount) }, 0) | decimal }}</td>
							<td style="text-align:right;">{{ reportData.reduce((prev, cur) => { return prev + parseFloat(cur.profit_loss) }, 0) | decimal }}</td>
						</tr>
					</tbody>
				</table>
			</div>
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			async getProfitLoss() {
				if(this.searchBy == 'customer'){
					this.filter.customer = this.selectedCustomer != null ? this.selectedCustomer.Customer_SlNo : null;
					...
					this.filter.supplierId = this.selectedSupplier != null ? this.selectedSupplier.Supplier_SlNo : null;
					this.reportData = await axios.post('/get_profit_loss_bysupplier', this.filter).then(res => {
						return res.data;
					})
				}
