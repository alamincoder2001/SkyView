...
class Page extends CI_Controller
{
    ...
    public function saveSubCategory()
    {
        ...
        $subCategoryId = $inputObJ->SubCat_ID;

        if ($subCategoryId == '') {
            $query = $this->db->query("select * from tbl_productsubcategory where Category_ID = ? and SubCategoryName = ? and branchid = ?", [$inputObJ->Category_ID, $inputObJ->SubCategoryName, $this->brunch])->row();
        } else {
            $query = $this->db->query("select * from tbl_productsubcategory where SubCat_ID != ? and Category_ID = ? and SubCategoryName = ? and branchid = ?", [$subCategoryId, $inputObJ->Category_ID, $inputObJ->SubCategoryName, $this->brunch])->row();
        }

        if ($query) {
            $res = ['success' => false, 'message' => 'Duplicate Sub Category Name'];
        ...
        try {
            ...
            if ($subCategoryId == '') {
                ...
                $data['branchid'] = $this->brunch;

                $this->db->insert('tbl_productsubcategory', $data);
            } else {
                unset($data['SubCat_ID']);
                $data['UpdateBy']   = $this->session->userdata("userId");
                $data['updatetime'] = date('Y-m-d H:i:s');
                $this->db->where('SubCat_ID', $subCategoryId)->update('tbl_productsubcategory', $data);
            }
    ...
    public function getSubCategories()
    {
        ...
        if (isset($data->categoryId) && $data->categoryId != '') {
            $clause .= " and psc.Category_ID = '$data->categoryId' ";
        }
        // if (isset($data->memo_for) && $data->memo_for == 'payment') {
        //     $clause = " and memo_for = 'payment' ";
        // }

        $categories = $this->db->query("SELECT psc.*,pc.ProductCategory_Name
            FROM tbl_productsubcategory psc
            LEFT JOIN tbl_productcategory pc on pc.ProductCategory_SlNo = psc.Category_ID
            WHERE psc.status = 'a'
            $clause
    ...
    public function deleteSubCategory()
    {
        ...
        $this->db->where('SubCat_ID', $data->SubCat_ID)->update('tbl_productsubcategory', $updateData);

        echo json_encode(['success' => true, 'message' => 'Item delete successfully']);
    }
